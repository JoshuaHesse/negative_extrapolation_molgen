...
CONTROL_MODEL = "full_model_norm_matched_random_corrected_neon_lambda_2p5"
ANALYSIS_MODELS = [*MODEL_ORDER, CONTROL_MODEL]
MODEL_LABELS = {
    "base": "Base",
...
def write_report(
    ...
    lines = [
        ...
        "",
        "The focal method is CNE at lambda 2.5. Comparisons are paired by seed.",
        "The statistics in this auxiliary report are retained as descriptive diagnostics for the SemlaFlow analysis outputs.",
        "Confirmatory inference uses the prespecified exact paired sign-flip tests and Holm families generated by scripts/analyze_paper_statistics.py under results/paper_statistics/.",
        "",
        header,
        separator,
        *rows,
...
def main() -> None:
    ...
    metrics[metrics["model"].isin(ANALYSIS_MODELS)].to_csv(
        output_dir / "endpoint_2d_plot_data.csv", index=False
    )
    pose[pose["model"].isin(ANALYSIS_MODELS)].to_csv(
        output_dir / "posebusters_plot_data.csv", index=False
    )
    diversity[diversity["model"].isin(ANALYSIS_MODELS)].to_csv(
        output_dir / "diversity_plot_data.csv", index=False
    )
    distance[
        distance["model"].isin(ANALYSIS_MODELS)
